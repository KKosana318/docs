...
To use Fix AI, you first create a flow. That flow can then be run on a set schedule or ad-hoc. Each time a flow is run, a detailed replay of the run with videos of each step being executed along with any bugs that are caught, is visible in the dashboard.
...
<Steps>
    ...
    3. Type in "fixaibot@gmail.com"
    4. Click the password input field
    5. Type in "password"
    6. Click the sign in button
    7. Verify that the user is signed in and the internal dashboard is visible
</Steps>

Here's an example of a flow built using Fix AI:
<Frame>
  <img src="/images/flow-page.png" alt="Example of a Fix AI flow" />
</Frame>
...
After a run completes, it can be viewed on the run replay page. Here is an example of a run on Fix AI:
<Frame>
  <img src="/images/run-replay.png" alt="Example of a Fix AI run replay" />
</Frame>
...
In addition, bugs can be sent to you directly via Slack or Linear. We can also provide additional integrations on request.
...
<Steps>
    1. **Flow Planning** - We work with you to create a list of all user flows that need to be tested, typically over Slack or a quick call.
    2. **Flow Creation** - We'll build all your flows for you. This process typically takes 1-3 days.
    3. **Scheduling Runs** - We'll schedule your flows to run at your preferred cadence.
    4. **Ongoing Support** - When new flows are needed or existing flows need changes, we can make those changes for you on an ad-hoc basis. Our turnaround time is typically less than a day.
</Steps>
